                        ...
                        sessions.map((session, index) =>
                            <div className="list-group-item slim" key={index}>
                                <div>
                                    ...
                                    <span className="session-list right">
                                        <button type="button" className="btn btn-default" onClick={() => openSession(session.id, session.isprivate)}>Open</button>
                                        <button type="button" className="btn btn-default" onClick={() => joinSession(session.id)}>Join</button>
                                    </span>
                                </div>
                                {
                                            ...
                                            <form className="form-inline">
                                                <div className={"form-group has-feedback" + (passwordError ? " has-error" : '')}>
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => joinPasswordCheck(e.target.value, session.id)}
                                                        value={joinPassword}
                                                    />
                                                    {passwordError ? <span className="glyphicon glyphicon-remove form-control-feedback"></span> : ''}
                                                </div>
                                                <button type="button" className="btn btn-default" onClick={() => continueToSession(session.id)}>Continue</button>
